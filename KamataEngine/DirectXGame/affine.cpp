...
void affineRollX(Vector3 vec[], float roll_)
{
	...
	for (int i = 0; i < 4; i++) {
		...
		vec[i].x =
			roll[0][0] * squareRollX + roll[0][1] * squareRollX + roll[0][2] * squareRollX;
		vec[i].y =
			roll[1][0] * squareRollY + roll[1][1] * squareRollY + roll[1][2] * squareRollY;
		vec[i].z =
			roll[2][0] * squareRollZ + roll[2][1] * squareRollZ + roll[2][2] * squareRollZ;
